...
let
  ...
  recordingToX = pkgs.writeShellApplication {
    ...
    text =
      renderScript ./recording-to-x.sh
  ...
  gpuScreenRecorderAutoX = pkgs.writeShellApplication {
    ...
    text =
      renderScript ./gpu-screen-recorder-auto-x.sh
...
in
{
  ...
  ++ lib.optionals (hostName == "attodesk") [
    # Shadow the recorder binary so Noctalia transparently starts the X-compatible transcode.
